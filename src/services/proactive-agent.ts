...
Only set hasTrend=true if there's genuinely something worth posting about TODAY. Don't force it.`;
  ...
  async sendWeeklyRetro(): Promise<void> {
    try {
      ...
      // Voice evolution: count this week's edit episodes
      const weekEditEpisodes = await this.memory.getRecentEpisodes("edit", 7);
      const weekApproveEpisodes = await this.memory.getRecentEpisodes("approve", 7);
...
Voice learning this week:
- Edits: ${weekEditEpisodes.length} posts edited (I'm learning from each one)
- Approvals without edits: ${weekApproveEpisodes.length} (matched your voice!)
${voiceProfile ? `- Voice profile confidence: ${(voiceProfile.confidence * 100).toFixed(0)}% (${voiceProfile.episode_count} total interactions)` : "- Voice profile: not yet built (need more interactions)"}
</week_data>
...
Respond in JSON:
{
  ...
  "voiceEvolution": "One sentence about how the agent's understanding of the user's voice evolved this week (mention specific edit patterns if any)",
...
Be honest but encouraging. If it was a slow week, acknowledge it and motivate.`;
      ...
      const blocks: KnownBlock[] = [
        ...
        ...(retro.voiceEvolution ? [{
          type: "section" as const,
          text: {
            type: "mrkdwn" as const,
            text: `:brain: *Voice Learning:* ${retro.voiceEvolution}`,
          },
        }] : []),
        { type: "divider" },
        {
...
Keep it to ONE punchy sentence. Make it feel like a high-five from a friend.`;
  ...
  private async getPersonalityContext(): Promise<string> {
    ...
    // Use structured voice profile if available
    if (voiceProfile) {
      sections.push(
        `\nLearned voice style (${(voiceProfile.confidence * 100).toFixed(0)}% confidence from ${voiceProfile.episode_count} interactions):`,
        `- Tone: ${voiceProfile.general.tone}`,
        `- Formality: ${voiceProfile.general.formality_level}`,
        `- Humor: ${voiceProfile.general.humor_style}`,
      );
      if (voiceProfile.never_do.length > 0) {
        sections.push(`- Never: ${voiceProfile.never_do.slice(0, 3).join(", ")}`);
      }
      if (voiceProfile.always_do.length > 0) {
        sections.push(`- Always: ${voiceProfile.always_do.slice(0, 3).join(", ")}`);
      }
    } else {
      // Fallback to raw style guide
